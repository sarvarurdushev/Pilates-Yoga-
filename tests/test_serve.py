...
class TestWhatIsOnDiskIsWhatIsOnScreen:
    ...
    def test_the_geometry_is_not_revalidated_every_load(self, running):
        """The rule above was applied to 25.8 MB of GLB as well, and those are
        not edited -- they are regenerated by a build script. Eight conditional
        requests per reload to be told nothing changed is a cost with nothing
        on the other side of it, and `web/render.yaml` already gave them a week
        on the static deployment, so the two were disagreeing."""
        base, _ = running
        headers, _ = self._head(f"{base}/models/nervous.glb")
        assert headers.get("Cache-Control") == "public, max-age=604800"

    def test_the_vendored_library_gets_the_same(self, running):
        base, _ = running
        headers, _ = self._head(f"{base}/vendor/three.module.js")
        assert headers.get("Cache-Control") == "public, max-age=604800"


class TestTheGeometryIsCompressed:
    """25.8 MB of model files, sent raw.

    Nothing in this server compressed anything, so every first load moved the
    whole atlas uncompressed. The GLBs gzip to 58% of their size, which is the
    single largest thing that can be done to how long this takes to open.
    """

    def _fetch(self, url, gzip_ok=True, extra=None):
    ...
    def test_a_model_arrives_compressed(self, running):
        base, _ = running
        headers, body = self._fetch(f"{base}/models/nervous.glb")
        assert headers.get("Content-Encoding") == "gzip"
        assert len(body) < 905312, "the compressed body is not smaller"

    def test_and_is_the_same_file_underneath(self, running):
        """The whole point, and the thing a wrong Content-Length breaks
        silently: what arrives has to decompress to the file on disk."""
        base, _ = running
        _, body = self._fetch(f"{base}/models/nervous.glb")
        raw = gzip.decompress(body)
        assert raw == (WEB / "models" / "nervous.glb").read_bytes()

    def test_it_still_says_what_the_payload_is(self, running):
        """`Content-Encoding` describes the transfer; `Content-Type` has to keep
        describing the payload. Sent as application/gzip the loader refuses it
        and the body never appears."""
        base, _ = running
        headers, _ = self._fetch(f"{base}/models/nervous.glb")
        assert headers.get("Content-Type") == "model/gltf-binary"

    def test_a_client_that_did_not_ask_gets_it_raw(self, running):
        base, _ = running
        headers, body = self._fetch(f"{base}/models/nervous.glb", gzip_ok=False)
        assert headers.get("Content-Encoding") is None
        assert body[:4] == b"glTF"

    def test_the_answer_varies_on_what_was_asked_for(self, running):
        """Without this a shared cache can hand the gzipped body to a client
        that never said it could read one."""
        base, _ = running
        for accepts in (True, False):
            headers, _ = self._fetch(f"{base}/models/nervous.glb", gzip_ok=accepts)
    ...
    def test_a_compressed_copy_reports_the_original_modification_time(self, running):
        ...
        file -- every browser holding it cached would be told to fetch again."""
        base, _ = running
        headers, _ = self._fetch(f"{base}/models/nervous.glb")
        request = urllib.request.Request(
            f"{base}/models/nervous.glb",
            headers={"Accept-Encoding": "gzip",
    ...
    def test_something_too_small_to_be_worth_it_is_left_alone(self, running):
        base, _ = running
        assert (WEB / "models" / "frame.json").stat().st_size < 4096
        headers, _ = self._fetch(f"{base}/models/frame.json")
        assert headers.get("Content-Encoding") is None

    def test_nothing_is_written_into_the_site_directory(self, running):
        """The module docstring promises it, and a .glb.gz appearing beside a
        .glb is how it ends up committed."""
        base, _ = running
        self._fetch(f"{base}/models/nervous.glb")
        assert not list(WEB.rglob("*.gz"))
