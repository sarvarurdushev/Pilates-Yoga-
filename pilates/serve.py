...
#: Extensions worth gzipping. GLB is the one that matters: the eight model files
#: are 25.8 MB and gzip to 15.0 MB, and this server was sending them raw.
COMPRESSIBLE = {".glb", ".json", ".js", ".mjs", ".css", ".html", ".svg", ".txt", ".map"}

#: Below this a gzip costs more in CPU and header bytes than it saves.
GZIP_MIN_BYTES = 4096

#: Paths whose contents change only when the build re-runs. The GLBs are
#: regenerated by ``scripts/build_body.py`` and the vendored three.js by a
#: dependency bump; neither is edited, so revalidating 25 MB of them on every
#: reload buys nothing. Everything else keeps ``no-cache`` -- see ``end_headers``
#: for why that matters more than it sounds.
IMMUTABLE_PREFIXES = ("/models/", "/vendor/")
...
class Handler(SimpleHTTPRequestHandler):
    ...
    #: ``"gzip"`` while this response is being served from a compressed copy.
    #: Set by ``translate_path`` and consumed by ``end_headers``, because those
    #: are the two hooks the base class gives between choosing a file and
    #: writing its headers.
    _encoding = ""

    #: Where the compressed copies live. Not in the site directory: the module
    #: docstring promises nothing is ever written there, and a ``.glb.gz``
    #: appearing next to a ``.glb`` is how it ends up committed. One temporary
    #: directory per process, shared across threads and removed with it.
    _gz_dir: str = ""
    ...
    @classmethod
    def _gz_root(cls) -> str:
        with cls._gz_lock:
            if not cls._gz_dir:
                cls._gz_dir = tempfile.mkdtemp(prefix="pilates-gz-")
            return cls._gz_dir

    def _gzipped(self, source: Path) -> Path | None:
        """A gzipped copy of ``source``, made on first request, or None.

        Compressed lazily rather than at startup because most of what this
        serves is never asked for in a given session -- a viewer that opens the
        skeleton and nothing else should not pay for the organs. The first
        request for each file pays once; the copy is reused until the file
        changes or the process ends.

        **The copy carries the source's modification time.** The base class
        answers conditional requests out of ``fstat`` on whatever file it opened,
        so a copy stamped with the time it was compressed would report itself
        newer than the thing it is a copy of, and every browser that had the file
        cached would be told to fetch it again.
        """
        try:
            stat = source.stat()
        except OSError:
            return None
        if source.suffix.lower() not in COMPRESSIBLE or stat.st_size < GZIP_MIN_BYTES:
            return None
        # keyed by identity and mtime, so an edited file is not served from a
        # stale copy -- this is the same failure the no-cache header exists for
        key = f"{stat.st_ino:x}-{int(stat.st_mtime_ns):x}-{source.name}.gz"
        out = Path(self._gz_root()) / key
        if out.exists():
            return out
        try:
            part = out.with_suffix(out.suffix + ".part")
            with source.open("rb") as raw, gzip.open(part, "wb", compresslevel=6) as packed:
                shutil.copyfileobj(raw, packed, 1 << 20)
            os.utime(part, ns=(stat.st_atime_ns, stat.st_mtime_ns))
            # atomic, so two threads racing the same first request cannot serve
            # each other a half-written file
            os.replace(part, out)
        except OSError:
            return None
        # A compressed copy that is not smaller is a copy that costs a decode
        # for nothing. Already-compressed payloads land here.
        if out.stat().st_size >= stat.st_size:
            return None
        return out

    def translate_path(self, path):
        """The file to open -- which may be a compressed copy of it.

        Done here rather than by answering the request directly because the base
        class's ``send_head`` already handles directory indexes, path traversal,
        ``If-Modified-Since`` and the 404, and reimplementing all of that to add
        one header is how a static server grows a security hole.
        """
        real = super().translate_path(path)
        self._encoding = ""
        if self.command not in ("GET", "HEAD") or not self._accepts_gzip():
            return real
        packed = self._gzipped(Path(real))
        if packed is None:
            return real
        self._encoding = "gzip"
        return str(packed)
    ...
    def guess_type(self, path):
        """The type of what the browser will have, not of what is on the wire.

        ``Content-Encoding`` describes the transfer; ``Content-Type`` has to keep
        describing the payload, or a gzipped ``.glb`` arrives as
        ``application/gzip`` and the loader refuses it.
        """
        name = str(path)
        if self._encoding == "gzip" and name.endswith(".gz"):
            name = name[:-3]
    ...
    def end_headers(self):  # noqa: N802
        ...
        **Except the geometry.** That rule was applied to everything, including
        25.8 MB of GLB, and those are not edited: they are regenerated by a
        build script, and between builds a conditional request for each of them
        is eight round trips to be told nothing changed. They get a week, which
        is what `web/render.yaml` already gives them on the static deployment --
        the two were disagreeing, and the server was the one that was wrong.
        """
        if not self._said_cache:
            path = urlparse(self.path).path
            immutable = path.startswith(IMMUTABLE_PREFIXES)
            self.send_header(
                "Cache-Control",
                f"public, max-age={IMMUTABLE_MAX_AGE}" if immutable else "no-cache")
        self._said_cache = False
        if self._encoding:
            self.send_header("Content-Encoding", self._encoding)
            self._encoding = ""
        # Whether a shared cache may hand this body to a client that did not ask
        # for gzip. Sent for every static answer, not only the compressed ones:
        # the uncompressed 200 is the one that must not be reused.
